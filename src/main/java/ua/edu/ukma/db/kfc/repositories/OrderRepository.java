...
public class OrderRepository extends BaseRepository<OrderEntity, Integer> {
    ...
    public Optional<OrderEntity> findById(Integer id) {
        String query = """
                SELECT o.id, o.cost, o.date_created, o.is_completed,
                    o.restaurant_id,
    ...
    public Integer save(OrderEntity entity) {
        String query = """
            INSERT INTO orders (cost, date_created, is_completed, restaurant_id, client_id, employee_id)
            VALUES (?, ?, ?, ?, ?, ?)
            RETURNING id
            """;
        try (PreparedStatement stmt = transactionManager.currentTransaction().prepareStatement(query)) {
            stmt.setBigDecimal(1, entity.getCost());
            stmt.setTimestamp(2, TimeUtils.mapToSqlTimestamp(entity.getDateCreated()));
            stmt.setBoolean(3, entity.isCompleted());
            stmt.setInt(4, entity.getRestaurantId());
            if (entity.getClientId() != null) stmt.setInt(5, entity.getClientId());
            else stmt.setNull(5, INTEGER);
            if (entity.getEmployeeId() != null) stmt.setInt(6, entity.getEmployeeId());
            else stmt.setNull(6, INTEGER);
            try (ResultSet rs = stmt.executeQuery()) {
